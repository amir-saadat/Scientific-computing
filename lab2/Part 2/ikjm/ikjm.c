...
/*  extern void dummy(void *);*/
  ...
  int retval;
  int i,j,k,l;
  int Num_Tot_n = 24;/*Number of different n used here*/
  FILE * InFile;
  ...
  double Am[n*n], Bm[n*n], Cm[n*n];/*Matrices*/
  ...
  for(i=0;i<n;i++)
	for(k=0;k<n;k++)
     for(j=0;j<n;j++)
	Cm[i*n+j]=Cm[i*n+j]+Am[i*n+k]*Bm[k*n+j];
  ...
  /*reporting the residual*/
    ...
    if (dim1==n){
     ...
     for (i = 0; i < n; i++) {
   	  for (j = 0; j < n; j++){
   	  	NormC += C_[i][j]*C_[i][j]; 	  	
   	    NormCmC_ += (C_[i][j]-Catlas[i][j])*(C_[i][j]-Catlas[i][j]);
   	    printf("DC[i,j]: %lf\n", (C_[i][j]-Catlas[i][j]));
   	  }
   	  }
